...
export class PlayerController {
  ...
  update(delta: number) {
    ...
    const eyesInWater = this.world.getVoxel(camPos.x, camPos.y, camPos.z) === BlockType.WATER;
    const feetInWater = this.world.getVoxel(feetPos.x, feetPos.y, feetPos.z) === BlockType.WATER;
    
    this.player.stats.isUnderwater = eyesInWater;
    const isSubmerged = eyesInWater || feetInWater;
    ...
    const speedMultiplier = isSubmerged ? 0.6 : 1.0;

    if (this.player.gameMode.hasGravity()) {
      if (isSubmerged) {
        this.velocity.y *= 0.9; 
        if (this.keys['Space']) {
          this.velocity.y = PLAYER_SPEED * 0.7; 
    ...
    if (this.keys['Space'] && this.canJump && this.player.gameMode.hasGravity() && !eyesInWater) {
