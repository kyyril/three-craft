...
export class Viewmodel {
  ...
  // Base transformations for idle state
  ...
  constructor(camera: THREE.Camera, atlas: THREE.Texture) {
    ...
    // Position the group relative to the camera
    // Adjusted for better visibility of the "hand" area
  ...
  private createToolModel(props: any, group: THREE.Group) {
    ...
    // Pivot group to rotate from the "bottom" of the handle
    const pivot = new THREE.Group();
    group.add(pivot);

    // Handle
    const handle = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.6, 0.04), handleMaterial);
    handle.position.y = 0.3; // Center of handle relative to pivot
    pivot.add(handle);
  ...
  public swing() {
    // Restart swing if already swinging or start a new one
  ...
  public update(delta: number, isMoving: boolean) {
    ...
    if (this.isSwinging) {
      ...
      if (this.swingProgress >= Math.PI) {
        ...
        // Arc movement: rotate forward and move slightly
    ...
    // Bobbing/Sway (Only if not in an active swing/equip phase to avoid jitter)
    if (!this.isSwinging && this.equipProgress >= 1) {
        ...
        // Reset base group pos during animation
        this.group.position.set(0.45, -0.4, -0.5);
    }
  }
